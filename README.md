...
- **Resposta:**
  ...
  **Conclusão:**  
  \[
  v((p \lor r) \to (q \lor r)) = T \quad \text{(independentemente do valor de \( v(r) \))}.
  \]
...
- **Resposta:**
  ...
  **Passo a passo:**
  ...
  3. **Analisando \( v(p \lor r)=T \):**  
     Com \( v(r)=F \) (obtido acima), para que \( v(p \lor r) \) seja verdadeiro, deve ser:
     - \( v(p)=T \).
...
- **Resposta:**
  ...
  **Conclusão:**  
  \[
  v_1(A \land (p \lor (p \land q))) = F \quad \text{e} \quad v_2(A \land (p \lor (p \land q))) = F.
  \]
